...
def get_shop_list_by_dishes_json(dishes,person_count):
    import json
    with open("book.json", "r", encoding = "cp1251") as f:
        my_file = json.load(f)
        shop_list = {}
        for dish in dishes:
            if dish in my_file.keys():
                for ingridient in my_file[dish]:
...
def create_file_json():
    import json
    with open("book.json", "w", encoding = "cp1251") as f:
...
def create_shop_list_json():
      ...
      dishes = input('Введите блюда в расчете на одного человека (через запятую): ') \
        .lower().split(', ')
      shop_list = get_shop_list_by_dishes_json(dishes, person_count)
